...
int bfs (int (&land)[MAX_NUM_ROWS][MAX_NUM_COLS], int num_rows, int num_cols, int r, int c);
...
int
bfs (int (&land)[MAX_NUM_ROWS][MAX_NUM_COLS], int num_rows, int num_cols, int r, int c)
{
  std::vector<std::pair<int, int>> d_moves = { { -1, 0 }, { 1, 0 }, { 0, -1 }, { 0, 1 } };
  ...
  while (!q.empty ()) {
    ...
    for (std::pair<int, int> d_move : d_moves) {
      ...
      if (moved_r >= 0 && moved_r < num_rows && moved_c >= 0 && moved_c < num_cols) {
