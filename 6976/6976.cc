...
main ()
{
  ...
  for (uint32_t i = 0; i < T; ++i) {
    ...
    while (long_digits.length () > 2) {
      std::string small = std::string (long_digits, long_digits.length () - 1, 1);
      std::cout << long_digits << "\n";
      long_digits.pop_back ();
      sub (long_digits, small);

      for (auto iter = long_digits.begin (); iter != long_digits.end (); ++iter) {
        if (*iter == '0')
    ...
    num = 0;
    mul = 1;
    for (auto iter = long_digits.begin (); iter != long_digits.end (); ++iter, mul *= 10) {
      num *= mul;
      num += (*iter - '0');
    }
...
void
sub (std::string &big, std::string small)
{
  uint32_t carry = 0;
