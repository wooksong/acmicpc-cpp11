...
main ()
{
  ...
  dict_shortcuts = std::unordered_map<int, std::vector<std::pair<int, int>>> (D + 1);
  for (int i = 0; i < N; ++i) {
    int from, to, dist;
  ...
  for (int i = 1; i <= D; ++i) {
    ...
    for (std::pair<int, int> s_with_d : dict_shortcuts[i]) {
      shortest[i] = std::min (shortest[i],
          std::min (shortest[i - 1] + 1, shortest[s_with_d.first] + s_with_d.second));
    }
  }
