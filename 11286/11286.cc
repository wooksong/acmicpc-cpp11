...
bool AbsHeap::has_lchild(size_t idx) {
    return (get_lchild_idx_of(idx) < m_size);
}
bool AbsHeap::has_rchild(size_t idx) {
    return (get_rchild_idx_of(idx) < m_size);
}
size_t AbsHeap::get_lchild_idx_of(size_t idx) { return idx * 2 + 1; }
size_t AbsHeap::get_rchild_idx_of(size_t idx) { return idx * 2 + 2; }
size_t AbsHeap::get_parent_idx_of(size_t idx) { return (idx - 1) / 2; }

void AbsHeap::heapify_up() {
    size_t idx;
    size_t idx_p;
