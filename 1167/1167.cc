...
find_longest_path (int from)
{
  ...
  while (!pq.empty ()) {
    ...
    for (std::pair<int, int> to_node : tree[from_node.first]) {
      ...
      if (longest_path[to_node.first] < longest_path[from_node.first] + to_node.second) {
        longest_path[to_node.first] = longest_path[from_node.first] + to_node.second;
        pq.push ({ to_node.first, longest_path[from_node.first] + to_node.second });
      }
    }
  }
}
