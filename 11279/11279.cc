...
bool MaxHeap::is_empty() { return (this->get_nums_data() == 0 ? true : false); }
size_t MaxHeap::get_nums_data() { return this->m_size; }
size_t MaxHeap::get_parent_idx_of(size_t idx) { return (idx - 1) / 2; }
size_t MaxHeap::get_lchild_idx_of(size_t idx) { return idx * 2 + 1; }
size_t MaxHeap::get_rchild_idx_of(size_t idx) { return idx * 2 + 2; }

bool MaxHeap::has_lchild(size_t idx) {
    return (this->get_lchild_idx_of(idx) < this->get_nums_data());
}

bool MaxHeap::has_rchild(size_t idx) {
...
void MaxHeap::heapify_up() {
    ...
    while (idx != 0) {
        ...
        std::swap(this->m_data[idx],
                  this->m_data[this->get_parent_idx_of(idx)]);
        idx = this->get_parent_idx_of(idx);
    }
}

void MaxHeap::heapify_down() {
    size_t idx;
    size_t r_idx, c_idx;
