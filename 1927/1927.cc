...
MinHeap::heapify_down ()
{
  ...
  while (idx != -1) {
    ...
    if (l_idx != -1) {
      c_idx = l_idx;
      if (r_idx != -1
          && this->m_data[c_idx] != std::min (this->m_data[l_idx], this->m_data[r_idx])) {
        c_idx = r_idx;
      }
    }
...
int
MinHeap::get_idx_parent_of (int idx)
{
  return (idx - 1) / 2;
}
int
MinHeap::get_idx_lchild_of (int idx)
{
  return idx * 2 + 1;
}
int
MinHeap::get_idx_rchild_of (int idx)
{
  return idx * 2 + 2;
}

bool
MinHeap::has_lchild (int idx)
{
  return (get_idx_lchild_of (idx) < (this->m_size));
}

bool
MinHeap::has_rchild (int idx)
{
  return (get_idx_rchild_of (idx) < (this->m_size));
}

void
MinHeap::insert (int n)
{
